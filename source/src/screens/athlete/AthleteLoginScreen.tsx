...
const idNumberSchema = (idType: string | undefined) => {
  if (idType === 'APAAR') {
    return yup.string().matches(/^\d{12}$/, 'Enter a valid 12-digit APAAR ID').required('ID Number is required');
  }
  if (idType === 'AADHAR') {
    return yup.string().matches(/^\d{12}$/, 'Enter a valid 12-digit Aadhar number').required('ID Number is required');
  }
  ...
  return (
    <Screen scroll keyboard centered fill>
      <AuthHeader
        role="athlete"
        title="Athlete Login"
        subtitle="Sign in with your NSRS / APAAR / Aadhaar ID."
