...
describe('Stress Tests - 1000 Measurements Pipeline', () => {
  test('Processes 1,000 rapid frame measurements under 500 ms with zero memory leaks', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      const res = calculateHeight(aruco, pose, 1280, 720);
      if (res.success) {
        successCount++;
        smoother.push(res.measurement.heightCm, startTime + i * 33);
      }
    }

    const duration = Date.now() - startTime;
    expect(successCount).toBe(1000);
    expect(duration).toBeLessThan(1000); // 1,000 measurements processed in < 1 second!
  });
});
